package com.bigdata.flink.streaming.sink
...
object StreamingDataToRedisScala {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //链接socket获取输入数据
    val text = env.socketTextStream("node1", port, '\n')
    ...
    val conf = new FlinkJedisPoolConfig.Builder().setHost("node2").setPort(6379).build()
